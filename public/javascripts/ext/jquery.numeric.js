...
$.fn.numeric.keypress = function(e)
{
  ...
  if(key < 48 || key > 57)
  {
    ...
    if(decimal && (key == decimal.charCodeAt(0) || key == decimal.charCodeAt(1)) && (value.indexOf(decimal[0]) != -1 && value.indexOf(decimal[1]) != -1))
    {
     allow = false;
    }
    ...
    if(decimal && (key == decimal.charCodeAt(0) || key == decimal.charCodeAt(1)))
    {
      if(value.indexOf(decimal[0]) == -1 && value.indexOf(decimal[1]) == -1)
      {
        allow = true;
      }
      else
...
$.fn.numeric.keyup = function(e)
{
  ...
  if(val && val.length > 0)
  {
    ...
    var lastDecimal = Math.max(val.lastIndexOf(decimal[0]), val.lastIndexOf(decimal[1]));
    if (lastDecimal != -1) {
      var firstPart = val.substring(0, lastDecimal);
...
$.fn.numeric.blur = function()
{
  ...
  if(val != "")
  {
    var re1 = new RegExp("^\\d+$|\\d*" + decimal[0] + "\\d+");
    var re2 = new RegExp("^\\d+$|\\d*" + decimal[1] + "\\d+");

    if(!re1.exec(val) || !re2.exec(val))
    {
      callback.apply(this);
    }
  }
